...
const VantaBackground = ({ sectionConfig, className }: VantaBackgroundProps) => {
  ...
  useEffect(() => {
    if (!sectionConfig || !sectionConfig.enabled || !vantaRef.current) {
      return;
    }

    const effectName = sectionConfig.effect?.toUpperCase();
    if (!effectName || !window.VANTA || !window.VANTA[effectName]) {
      return;
    }
  ...
  // Effect for updating colors on theme change
  useEffect(() => {
    if (vantaEffect && sectionConfig?.enabled) {
      const isDark = resolvedTheme === 'dark';
  ...
  }, [resolvedTheme, vantaEffect, sectionConfig]);
