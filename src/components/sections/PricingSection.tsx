    ...
    <section id="pricing" className="relative">
      ...
      <div className="container">
        <div className="text-center">
          <h2 className="text-3xl font-bold md:text-4xl" style={useVanta ? { color: 'white', textShadow: '0 2px 4px rgba(0,0,0,0.5)' } : {}}>Flexible Pricing Plans</h2>
          <p className="mx-auto mt-4 max-w-2xl text-lg text-muted-foreground" style={useVanta ? { color: 'white', textShadow: '0 1px 3px rgba(0,0,0,0.4)' } : {}}>
            Choose a plan that fits your needs. All plans include one year of free support.
          </p>
        </div>
        <div className="mt-12 grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
          {pricingPlans.map((plan) => (
            <Card
              key={plan.title}
              className={cn('flex flex-col', plan.is_featured && 'border-2 border-primary shadow-lg')}
            >
              <CardHeader>
              ...
              <CardFooter>
                <Button asChild className="w-full" variant={plan.is_featured ? 'default' : 'outline'}>
                  <Link href={plan.title.toLowerCase().includes('custom') ? '/contact' : `/checkout?plan=${encodeURIComponent(plan.title)}&price=${parsePrice(plan.price)}`}>
