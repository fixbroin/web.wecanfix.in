      ...
      <div className="container py-12 relative z-10">
        <div className="grid grid-cols-1 gap-8 md:grid-cols-4">
          <div className="space-y-4 col-span-1 md:col-span-2">
            <Logo appName={settings?.website_name} />
            <p className="text-sm max-w-md" style={useVanta ? { color: 'white', textShadow: '0 1px 3px rgba(0,0,0,0.4)' } : {}}>
              Crafting high-performance websites with modern technology.
            </p>
          </div>
          <div className="md:justify-self-center">
            ...
            <ul className="mt-4 space-y-2">
              {navLinks.map((link) => (
                <li key={link.href}>
                  <Link href={link.href} className="text-sm hover:text-primary transition-colors" style={useVanta ? { color: 'white', textShadow: '0 1px 3px rgba(0,0,0,0.4)' } : {}}>
                    {link.label}
                  </Link>
                </li>
              ))}
          ...
          <div className="md:justify-self-center">
            ...
            <div className="mt-4 flex space-x-4">
              {socialLinks.map((link, index) => (
                <Link key={index} href={link.href || '#'} target="_blank" rel="noopener noreferrer" className="text-secondary-foreground hover:text-primary transition-colors" style={useVanta ? { color: 'white' } : {}}>
        ...
        <div className="mt-8 border-t pt-6 text-center text-sm text-muted-foreground flex flex-col sm:flex-row justify-between items-center gap-4" style={useVanta ? { color: 'white', textShadow: '0 1px 3px rgba(0,0,0,0.4)' } : {}}>
          <p>&copy; {new Date().getFullYear()} {settings?.website_name || 'WebDesignBro'}. All rights reserved.</p>
           <ul className="flex flex-wrap justify-center gap-x-4 gap-y-2">
              {legalLinks.map((link) => (
                <li key={link.href}>
                  <Link href={link.href} className="text-sm hover:text-primary transition-colors" style={useVanta ? { color: 'white' } : {}}>
                    {link.label}
                  </Link>
                </li>
              ))}
