...
export default function ImageUploadInput({ id, value, onChange, className, accept = "image/png, image/jpeg, image/webp, image/svg+xml, image/ico, .ico" }: ImageUploadInputProps) {
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!acceptedTypes.includes(file.type) && !acceptedTypes.includes(file.type.split('/')[0] + '/*')) {
         toast({
            variant: 'destructive',
    ...
    <div className={cn("space-y-4", className)}>
        ...
        <div className="flex items-center gap-4">
            ...
            <label htmlFor={id} className={cn(buttonVariants({ variant: 'outline' }), 'cursor-pointer w-full')}>
                ...
                {isPending && uploadProgress !== null ? `Uploading ${Math.round(uploadProgress)}%` : 'Change Media'}
            </label>
        </div>
