...
export interface WhyChooseUsContent {
    ...
    subtitle: string;
    image: string;
    features: WhyChooseUsFeature[]; // The type now uses the updated WhyChooseUsFeature
}
...
export async function getWhyChooseUsContent(): Promise<WhyChooseUsContent> {
    try {
        ...
        if (contentSnap.exists()) {
            contentData = contentSnap.data() as Omit<WhyChooseUsContent, 'features'>;
        } else {
            contentData = {
...
export async function updateWhyChooseUsContent(content: Omit<WhyChooseUsContent, 'features'> & { features: Omit<WhyChooseUsFeature, 'id' | 'createdAt'>[] }): Promise<void> {
    try {
        ...
        content.features.forEach(feature => {
            ...
            // Feature from the form won't have createdAt, so we add it here.
